...
export function buildMergeData({
  ...
  return {
    ...
    firma_iletisim_line: [companySettings?.phone, companySettings?.email]
      .filter(Boolean)
      .join("  ·  "),
    ...
    gecerlilik_metni: noExpiry
      ? `Bu ${noun} süresiz geçerlidir.`
      : `Bu ${noun} ${validityDays || 15} gün geçerlidir.`,
...
export function renderTemplateBlocks(blocks, mergeData, lineItems = []) {
  ...
  const items =
    lineItems.length > 0
      ? lineItems
      : [{ description: mergeData.teklif_basligi, quantity: 1, unitPrice: 0 }];
  const tableBlock = blocks.find((b) => b.type === "table");
  const extraRows = Math.max(0, items.length - 1);
  const shiftAmount = extraRows * TABLE_ROW_HEIGHT;
  const yOf = (b) =>
    tableBlock && b.id !== tableBlock.id && b.y > tableBlock.y ? b.y + shiftAmount : b.y;
        ...
        return (
          <tr
            ...
            style={{
              ...
              background: b.accentColor ? "#f5f8fc" : "transparent",
            }}
          >
            <td
              style={{
            ...
            <td style={{ padding: "9px 0", fontSize: 14, textAlign: "right", color: "#0c2540" }}>
              {formatTL(net)}
            </td>
          </tr>
        );
...
export const MERGE_FIELD_OPTIONS = [
  ...
  { key: "belge_basligi", label: "Belge Başlığı (Teklif/Randevu/Üyelik/Rezervasyon)" },
...
const SAMPLE_COMPANY_SETTINGS = {
  companyName: "Örnek A.Ş.",
  address: "Mevlana Mah. 1700. Sok. No 42/4 Önder Sitesi E Blok, Konyaaltı/Antalya",
  phone: "0553 062 43 99",
...
export const SAMPLE_LINE_ITEMS = [
  { description: "Web Sitesi Yenileme", quantity: 1, unitPrice: 5000 },
];
...
export function TemplateGallery({
  ...
  const builtIn = Object.entries(PDF_TEMPLATES).map(([key, tpl]) => ({
    key,
    id: null,
    label: tpl.label,
    width: tpl.width,
      ...
      <button
        ...
        onClick={() =>
          onCreateNew({
            id: null,
            name: "",
            width: BLANK_TEMPLATE_WIDTH,
            height: BLANK_TEMPLATE_HEIGHT,
            blocks: [],
          })
        }
        style={{
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        {all.map((tpl) => (
          ...
          >
              ...
              <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                ...
                <button
                  ...
                  onClick={() =>
                    onEdit({
                      id: tpl.id,
                      name: tpl.id ? tpl.label : `${tpl.label} (Kopya)`,
                      width: tpl.width,
                      height: tpl.height,
                      blocks: tpl.blocks,
                    })
            ...
            <div
              style={{
                width: tpl.width * GALLERY_SCALE,
                height: tpl.height * GALLERY_SCALE,
                overflow: "hidden",
                position: "relative",
                background: "#fff",
                border: "0.5px solid var(--border)",
