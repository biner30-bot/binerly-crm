...
function ConnectPanel({ channel, credential, onSave, onDisconnect, onCancel }) {
  ...
  const verifyToken =
    import.meta.env.VITE_META_WEBHOOK_VERIFY_TOKEN || "(Binerly'nin size ileteceği doğrulama kodu)";
  ...
  return (
    <Modal title={`${CHANNEL_LABELS[channel]} bağlantısı`} onClose={onCancel}>
      ...
      >
        <p
          style={{ margin: "0 0 6px", fontSize: 13, fontWeight: 500, color: "var(--text-accent)" }}
        >
          Bu bağlantı tamamen ücretsizdir
        </p>
      ...
      {credential ? (
        <div style={{ marginBottom: 16 }}>
          ...
          <button
            ...
            style={{ fontSize: 13, color: "var(--text-danger)" }}
          >
            Bağlantıyı kaldır
          </button>
        </div>
        ...
        <form onSubmit={submit}>
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            <input
              ...
              placeholder="Örn. Mağaza WhatsApp hattı"
              style={{ width: "100%" }}
            />
          </div>
          ...
          >
            <p style={{ margin: "0 0 4px" }}>
              Meta Geliştirici panelinizde webhook olarak şunu girin:
            </p>
            <p style={{ margin: "0 0 4px", fontFamily: "monospace", wordBreak: "break-all" }}>
              {webhookUrl}
            </p>
            <p style={{ margin: 0 }}>
              Doğrulama kodu (Verify Token):{" "}
              <span style={{ fontFamily: "monospace" }}>{verifyToken}</span>
            </p>
          </div>
        ...
        return (
          <div
            ...
            style={{
              ...
              borderRadius: "var(--radius)",
              cursor: "pointer",
              background: selectedKey === key ? "var(--surface-2)" : "transparent",
            }}
          >
            <i
              className={`ti ${c.channel === "whatsapp" ? "ti-brand-whatsapp" : "ti-brand-instagram"}`}
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              >
                {c.lastMessage.direction === "out" ? "Siz: " : ""}
                {c.lastMessage.body}
              </p>
            </div>
...
function ThreadView({ conversation, customer, onSend, onConvertToCustomer, onCreateDeal }) {
  ...
  const withinWindow =
    !!lastInbound && Date.now() - new Date(lastInbound.createdAt).getTime() <= 24 * 3600 * 1000;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {!conversation.customerId && conversation.channel === "whatsapp" && (
            <button onClick={onConvertToCustomer} style={{ fontSize: 12 }}>
              Müşteriye dönüştür
            </button>
          )}
          {conversation.customerId && (
            <button onClick={onCreateDeal} style={{ fontSize: 12 }}>
              Teklif oluştur
            </button>
          )}
        </div>
      ...
      <form onSubmit={submit} style={{ display: "flex", gap: 8 }}>
        ...
        <button
          type="submit"
          disabled={sending || !body.trim() || !withinWindow}
...
export default function Messages({
  ...
  const selected =
    conversations.find((c) => `${c.channel}:${c.counterpartId}` === selectedKey) || null;
    ...
    <div>
      <div style={{ display: "flex", gap: 12, marginBottom: 16, flexWrap: "wrap" }}>
        {["whatsapp", "instagram"].map((channel) => {
          ...
          return (
            ...
            >
              <i
                className={`ti ${channel === "whatsapp" ? "ti-brand-whatsapp" : "ti-brand-instagram"}`}
              ...
              <div style={{ flex: 1 }}>
                ...
                <p
                  style={{
                    margin: 0,
                    fontSize: 12,
                    color: cred ? "var(--text-success)" : "var(--text-muted)",
                  }}
                >
                  {cred
                    ? `Bağlı${cred.displayName ? ` · ${cred.displayName}` : ""}`
                    : "Bağlı değil"}
                </p>
              </div>
      ...
      {!anyConnected ? (
        <p style={{ fontSize: 14, color: "var(--text-secondary)" }}>
          Gelen kutusunu kullanmaya başlamak için yukarıdan WhatsApp veya Instagram hesabınızı
          bağlayın.
        </p>
      ...
      {showConnect && (
        <ConnectPanel
          ...
          onSave={async (fields) => {
            await onSaveCredential(showConnect, fields);
            setShowConnect(null);
          }}
          onDisconnect={async () => {
            await onDisconnectCredential(showConnect);
            setShowConnect(null);
          }}
