...
export default async function handler(req, res) {
  ...
  const { data: cred } = await supabaseAdmin
    ...
    .eq("channel", "whatsapp")
    .maybeSingle();
  if (!cred) return res.status(400).json({ error: "WhatsApp bağlantısı yapılandırılmamış." });
  ...
  if (!withinWindow) {
    return res.status(400).json({ error: "24 saatlik yanıt penceresi kapandı. Bu numaraya şu an serbest metinli mesaj gönderilemez." });
  }
  ...
  if (!metaRes.ok) {
    return res.status(502).json({ error: "WhatsApp gönderimi başarısız.", detail: metaData?.error?.message });
  }
  ...
  await supabaseAdmin.from("channel_messages").insert({
    ...
    channel: "whatsapp",
    direction: "out",
    external_message_id: metaData?.messages?.[0]?.id || null,
    counterpart_id: to,
